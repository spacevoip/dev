...
export default function DashAgente() {
  ...
  const formatDuration = (duration: string | number) => {
    return typeof duration === 'string' ? duration : `${Math.floor(duration / 60)}:${(duration % 60).toString().padStart(2, '0')}`;
  };

  const handleHangup = async (channel: string) => {
    // Implementar lógica de desligar
    console.log('Desligar canal:', channel);
  };

  const handleTransfer = async (channel: string) => {
    // Implementar lógica de transferência
    console.log('Transferir canal:', channel);
  };
