...
export const useSupabaseQuery = <T extends { accountid?: string }>(
  ...
  const fetchData = async () => {
    if (!currentUser?.accountid && !options.skipAccountFilter) {
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // Adiciona o filtro de accountid apenas se não for skipado
      if (!options.skipAccountFilter) {
        query = query.eq('accountid', currentUser.accountid);
      }
